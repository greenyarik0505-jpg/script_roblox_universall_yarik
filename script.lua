...
local iconDragStartX = 0
local iconDragStartY = 0
local iconStartOffX = 0
local iconStartOffY = 0

IconFrame.InputBegan:Connect(function(inp)
	if inp.UserInputType == Enum.UserInputType.MouseButton1 then
		iconDragging = true
		iconMoved = false
		iconDragStartX = inp.Position.X
		iconDragStartY = inp.Position.Y
		iconStartOffX = IconFrame.Position.X.Offset
		iconStartOffY = IconFrame.Position.Y.Offset
	end
end)

IconFrame.InputEnded:Connect(function(inp)
	if inp.UserInputType == Enum.UserInputType.MouseButton1 then
		if not iconMoved then
			State.menuOpen = not State.menuOpen
			local MF = ScreenGui:FindFirstChild("MainFrame")
			if MF then
				MF.Visible = State.menuOpen
			end
		end
		iconDragging = false
		iconMoved = false
...
UIS.InputChanged:Connect(function(inp)
	if iconDragging and inp.UserInputType == Enum.UserInputType.MouseMovement then
		local dx = inp.Position.X - iconDragStartX
		local dy = inp.Position.Y - iconDragStartY
		if math.abs(dx) > 6 or math.abs(dy) > 6 then
			iconMoved = true
		end
		if iconMoved then
			local vp = Camera.ViewportSize
			local nx = math.clamp(iconStartOffX + dx, 0, vp.X - 54)
			local ny = math.clamp(iconStartOffY + dy, 0, vp.Y - 54)
