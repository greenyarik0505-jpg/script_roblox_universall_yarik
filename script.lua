...
IconButton.Position = UDim2.new(0, 20, 0, 300)
...
IconButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        iconMoved = false
        iconMouseDownPos = Vector2.new(input.Position.X, input.Position.Y)
    end
end)
...
UserInputService.InputChanged:Connect(function(input)
    ...
    local delta = currentPos - iconDragStart
    ...
    if iconMoved then
        local newX = math.clamp(iconStartPos.X.Offset + delta.X, 0, Camera.ViewportSize.X - 58)
        local newY = math.clamp(iconStartPos.Y.Offset + delta.Y, 0, Camera.ViewportSize.Y - 58)
